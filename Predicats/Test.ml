...
let test (ps : formule_syllogisme list) (c : formule_syllogisme) : unit =
  ...
  let dps = List.concat (List.map (fun p -> diag_from_formule ["a"; "b"; "c"] p) ps) in
  print_endline "Diagramme de chaque prémisse :";
  List.iter (fun d -> print_endline (string_of_diag d)) dps;

  let comb_dps = List.fold_left (fun acc dp -> conj_diag_list acc [ dp ]) [] dps in
  print_endline "Diagramme de la combinaison :";
  List.iter (fun d -> print_endline (string_of_diag d)) comb_dps;

  let dcs = diag_from_formule ["a"; "b"; "c"] c in
  print_endline "Diagramme de la conclusion :";
  ...
  else print_endline "Les prémisses ne sont pas compatibles avec la conclusion.";
