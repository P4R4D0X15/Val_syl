...
let test (ps : formule_syllogisme list) (c : formule_syllogisme) : unit =
  ...
  if compatible then
    ...
    let compl_dp1 =
      List.map
        (fun dp -> complete_diags dp [ "a"; "b"; "c" ])
        (diag_from_formule [ "a"; "b"; "c" ] p1)
    in
    print_endline "-p1:\n";
    List.iter
      (fun d -> print_endline (string_of_diag d))
      (diag_from_formule [ "a"; "b"; "c" ] p1);
    print_endline "-Complete diag sur p1:\n";
    List.iter
      (fun d -> print_endline (string_of_diag d))
      (List.concat compl_dp1)

let test_boolCombSyllogisme (b1 : boolCombSyllogismes)
    (b2 : boolCombSyllogismes) : unit =
