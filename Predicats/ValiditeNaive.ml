...
let undef (d : diagramme) (pl : Predicate_set.t list) : Predicate_set.t list =
  let rec aux acc pl = match pl with
  | [] -> acc
  | p::q when Diag.mem p d -> aux acc q
  | p::q -> aux (p::acc) q 
in aux [] pl

(** diag_from_undef d p : construit le liste des extensions possible du diagramme
     d sur une zone p*)
...
let complete_diags (d: diagramme) (ats : string list) : diagramme list = 
  ...
  List.fold_left (fun acc dl -> conj_diag_list acc dl) (List.hd d_list) (List.tl d_list)
...
let est_valid_premiss_conc (b1 : boolCombSyllogismes) (b2 : boolCombSyllogismes)
  ...
  in
  List.exists2 (fun d1 d2 -> not (is_contradiction d1 d2)) db2 db1
