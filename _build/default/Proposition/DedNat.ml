...
(** intro_disj_d a b : construit une preuve de a ∨ b à partir d'une preuve
        de a. *)
let intro_disj_d (p : arbre_preuve) (b : formule) : arbre_preuve =
  IntroDisjD (p, Ou(get_conc(p), b))

(** intro_disj_g b a : construit une preuve de a ∨ b à partir d'une preuve
        de b. *)
let intro_disj_g (p : arbre_preuve) (a : formule) : arbre_preuve =
  IntroDisjG (p, Ou(a, get_conc(p)))
...
let elim_imp (p1 : arbre_preuve) (p2 : arbre_preuve) : arbre_preuve option =
  match get_conc(p1), get_conc(p2) with
  | Imp(a', b), a when a = a' -> Some (ElimImp (p1,p2,b))
  | _ -> None
...
let elim_disj (p1 : arbre_preuve) (p2 : arbre_preuve) (p3 : arbre_preuve) :
    arbre_preuve option =
  match (get_conc(p1), get_conc(p2), get_conc(p3)) with
  | Ou (a, b), Imp(c, f),Imp(d, f') when a = c && b = d && f = f' -> Some (ElimDisj (p1, p2, p3, f))
  | _ -> None

(** elim_bot ⊥ f : construit une preuve de f à partir d'une preuve de
    ⊥. Renvoie None si le paramètre est incorrect. *)
let elim_bot (p : arbre_preuve) (f : formule) : arbre_preuve option = 
  match get_conc(p) with 
  | Bot -> Some (ElimBot(p, f))
  | _ -> None

(** elim_conj_d (a ∧ b) : construit une preuve de b à partir d'une preuve de
    a ∧ b. Renvoie None si le paramètre est incorrect. *)
let elim_conj_d (p : arbre_preuve) : arbre_preuve option = 
  match get_conc(p) with
  | Et (_, b) -> Some (ElimConjD (p, b))
  | _ -> None

(** elim_conj_g (a ∧ b) : construit une preuve de a à partir d'une preuve de
    a ∧ b. Renvoie None si le paramètre est incorrect. *)
let elim_conj_g (p : arbre_preuve) : arbre_preuve option = 
    match get_conc(p) with
  | Et (a, _) -> Some (ElimConjG (p, a))
  | _ -> None

(* ***** Règle de l'introduction de l'implication. ***** *)

let rec proofCalc (f : formule) (p : arbre_preuve) = 
  match p with
  | Hypo f' when f' = f -> HypoDechargee f
  | Hypo f' | HypoDechargee f' -> Hypo f'
  | ElimImp (p1, p2, g) -> ElimImp ((proofCalc f p1), (proofCalc f p2), g)
  | ElimBot (p, g) -> ElimBot ((proofCalc f p), g)
  | ElimConjD (p, g) -> ElimConjD ((proofCalc f p), g)
  | ElimConjG (p, g) -> ElimConjG ((proofCalc f p), g)
  | ElimDisj (p1, p2, p3, g) -> ElimDisj ((proofCalc f p1), (proofCalc f p2), (proofCalc f p3), g)
  | IntroConj (p1, p2, g) -> IntroConj ((proofCalc f p1), (proofCalc f p2), g)
  | IntroDisjD (p, g) -> IntroDisjD ((proofCalc f p), g)
  | IntroDisjG (p, g) -> IntroDisjG ((proofCalc f p), g)
  | IntroImp (p, g) -> IntroImp ((proofCalc f p), g)


(** intro_imp f g : construit une preuve de f → g à partir d'une preuve de
    g. Décharge les occurences de f dans la preuve de g.*)
let intro_imp (f : formule) (p : arbre_preuve) : arbre_preuve = 
  IntroImp (proofCalc f p, f) 
